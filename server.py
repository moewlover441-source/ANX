...
# Nhiệm vụ xem quảng cáo (Gigapub). Để trống GIGAPUB_PROJECT_ID = ẩn nhiệm vụ này.
...
GIGAPUB_REWARD_UNITS = max(1, round(GIGAPUB_REWARD_MICRO * 1000 / MICRO_PER_1000)) if GIGAPUB_PROJECT_ID else 0
...
def me_payload(uid):
    ...
    return {
        ...
        "config": {
            ...
            "gigapub": {
                "project_id": GIGAPUB_PROJECT_ID,
                "reward": GIGAPUB_REWARD_UNITS,
...
def ads_token(user):
    ...
    if purpose not in ("mine_start", "mine_claim", "gigapub_watch"):
        raise ApiError("invalid_purpose", "Không hợp lệ.")
    if purpose == "gigapub_watch" and gigapub_today_count(user) >= GIGAPUB_DAILY_LIMIT:
...
@app.route("/api/tasks/gigapub/claim", methods=["POST"])
@api
def gigapub_claim(user):
    """Client gọi sau khi Gigapub báo đã xem xong quảng cáo VÀ đã xin được ad_token cho
    purpose 'gigapub_watch'. Hạn mức ngày và phần thưởng được chốt lại ở đây, không tin
    con số client tự gửi lên."""
    uid = user["id"]
    cooldown(uid, "gigapub", 1)
    if not GIGAPUB_PROJECT_ID:
        raise ApiError("not_configured", "Nhiệm vụ này chưa được bật.", 404)
    token = (request.get_json(silent=True) or {}).get("ad_token")
    if not consume_ad_token(uid, "gigapub_watch", token, enabled=True):
    ...
    with tx():
        ...
        add_balance(uid, GIGAPUB_REWARD_UNITS, "gigapub", None, earned=True)
    return {"reward": GIGAPUB_REWARD_UNITS, "me": me_payload(uid)}
