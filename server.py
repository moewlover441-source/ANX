...
ADSGRAM_BLOCK_ID = os.environ.get("ADSGRAM_BLOCK_ID", "")  # để trống = không bắt buộc xem quảng cáo
...
def consume_ad_token(uid, purpose, token):
    ...
    lần là mất luôn quyền, phải xem lại quảng cáo từ đầu."""
    if not ADSGRAM_BLOCK_ID:
        return True   # chưa cấu hình quảng cáo thì không bắt buộc
...
def ads_token(user):
    ...
    if purpose not in ("mine_start", "mine_claim"):
...
def mine_start(user):
    ...
    if not consume_ad_token(uid, "mine_start", (request.get_json(silent=True) or {}).get("ad_token")):
...
def mine_claim(user):
    uid = user["id"]
    cooldown(uid, "mine", 0.5)
    if not consume_ad_token(uid, "mine_claim", (request.get_json(silent=True) or {}).get("ad_token")):
