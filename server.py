...
- Trang quản trị /admin: chỉ mở được từ Telegram bằng đúng tài khoản ADMIN_ID.
...
GIGAPUB_REWARD_UNITS = 0   # tính lại bên dưới, sau khi biết GIGAPUB_REWARD_MICRO
AD_TOKEN_TTL = 120   # giây: token xác nhận đã xem quảng cáo có hiệu lực trong bao lâu
ADMIN_ID = int(os.environ.get("ADMIN_ID", "8766063561"))   # duy nhất Telegram ID này vào được /admin
ADSGRAM_BLOCK_ID = os.environ.get("ADSGRAM_BLOCK_ID", "")  # để trống = không bắt buộc xem quảng cáo khi đào

# Nhiệm vụ xem quảng cáo (Gigapub). Cần cả hai biến dưới mới bật được nhiệm vụ này.
GIGAPUB_PROJECT_ID = os.environ.get("GIGAPUB_PROJECT_ID", "")
GIGAPUB_SECRET_KEY = os.environ.get("GIGAPUB_SECRET_KEY", "")   # khoá bí mật lấy trong dashboard Gigapub, dùng để kiểm tra chữ ký phần thưởng
GIGAPUB_DAILY_LIMIT = int(os.environ.get("GIGAPUB_DAILY_LIMIT", 120))
GIGAPUB_REWARD_MICRO = int(Decimal(os.environ.get("GIGAPUB_REWARD_USDT", "0.0004")) * 1_000_000)  # USDT mỗi quảng cáo

# Nhiệm vụ xem quảng cáo (Monetix). Khác Gigapub: callback không có chữ ký xác thực, nên
# chỉ tin được tới mức "client báo đã xem" — chặn gian lận bằng token dùng-một-lần do
# server phát ra (giống cách làm với Adsgram), không có cơ chế xác minh mật mã như Gigapub.
MONETIX_API_KEY = os.environ.get("MONETIX_API_KEY", "")
MONETIX_DAILY_LIMIT = int(os.environ.get("MONETIX_DAILY_LIMIT", 80))
MONETIX_REWARD_MICRO = int(Decimal(os.environ.get("MONETIX_REWARD_USDT", "0.0002")) * 1_000_000)
ADMIN_CHAT_ID = os.environ.get("ADMIN_CHAT_ID") or str(ADMIN_ID)   # nơi nhận thông báo yêu cầu rút
ADMIN_SESSION_SECONDS = 4 * 3600                           # initData của admin chỉ có giá trị 4 giờ
...
def gigapub_active():
    return bool(GIGAPUB_PROJECT_ID and GIGAPUB_SECRET_KEY)


GIGAPUB_REWARD_UNITS = max(1, round(GIGAPUB_REWARD_MICRO * 1000 / MICRO_PER_1000)) if GIGAPUB_PROJECT_ID else 0  # dùng nội bộ; hiển thị cho client còn tuỳ gigapub_active()
MONETIX_REWARD_UNITS = max(1, round(MONETIX_REWARD_MICRO * 1000 / MICRO_PER_1000)) if MONETIX_API_KEY else 0
...
with LOCK:
    ...
    DB.executescript(
        ...
        CREATE TABLE IF NOT EXISTS users(
            ...
            gigapub_day TEXT,                         -- ngày (UTC, YYYY-MM-DD) của bộ đếm quảng cáo Gigapub bên dưới
            gigapub_count INTEGER NOT NULL DEFAULT 0, -- số quảng cáo Gigapub đã xem trong ngày đó
            monetix_day TEXT,
            monetix_count INTEGER NOT NULL DEFAULT 0,
            banned INTEGER NOT NULL DEFAULT 0,
            ban_reason TEXT,
            referred_by INTEGER,
            created_at REAL NOT NULL
        );
        CREATE TABLE IF NOT EXISTS gigapub_rewards(
            reward_id TEXT PRIMARY KEY,               -- rewardId Gigapub gửi, khoá để chặn xử lý trùng
            user_id INTEGER NOT NULL,
            amount TEXT NOT NULL,                     -- giá trị "amount" gốc từ Gigapub (dạng chuỗi, giữ nguyên để khớp chữ ký)
            units INTEGER NOT NULL,                    -- số xu thực tế đã cộng cho người dùng
...
with LOCK:
    ...
    for _name, _ddl in (
        ...
        ("ban_reason", "TEXT"),
        ("gigapub_day", "TEXT"),
        ("gigapub_count", "INTEGER NOT NULL DEFAULT 0"),
        ("monetix_day", "TEXT"),
        ("monetix_count", "INTEGER NOT NULL DEFAULT 0"),
    ):
...
_ad_tokens = {}   # (uid, purpose) -> (token, hết hạn) — bộ nhớ tạm, không cần bền vững qua khi khởi động lại


def issue_ad_token(uid, purpose):
    tok = hashlib.sha256(os.urandom(24)).hexdigest()
    if len(_ad_tokens) > 20000:
        _ad_tokens.clear()
    _ad_tokens[(uid, purpose)] = (tok, time.time() + AD_TOKEN_TTL)
    return tok


def consume_ad_token(uid, purpose, token, enabled=True):
    """True nếu token hợp lệ, đúng người, đúng mục đích, còn hạn — và tiêu luôn (dùng một lần).
    Một lần thử SAI không được xoá mất token ĐÚNG đang chờ dùng, nếu không người dùng gõ nhầm một
    lần là mất luôn quyền, phải xem lại quảng cáo từ đầu. `enabled=False` (quảng cáo tương ứng
    chưa được admin cấu hình) thì luôn cho qua, không bắt buộc xem."""
    if not enabled:
        return True
    cur = _ad_tokens.get((uid, purpose))
    if not cur:
        return False
    saved_tok, exp = cur
    if time.time() >= exp:
        del _ad_tokens[(uid, purpose)]
        return False
    if not (token and hmac.compare_digest(saved_tok, token)):
        return False   # sai token: giữ nguyên token đúng, cho phép thử lại
    del _ad_tokens[(uid, purpose)]   # đúng: dùng một lần rồi xoá
    return True
...
def gigapub_today_count(u):
    return u["gigapub_count"] if u["gigapub_day"] == today_str() else 0


def monetix_today_count(u):
    return u["monetix_count"] if u["monetix_day"] == today_str() else 0
...
def parse_init_data(init_data, max_age=86400):
...
def auth():
    ...
    if user["banned"]:
        ...
        raise ApiError("banned", "Tài khoản của bạn đã bị khoá." + (" Lý do: " + reason if reason else ""), 403)
    return user
...
def me_payload(uid):
    ...
    return {
        ...
        "config": {
            ...
            "gigapub": {
                "project_id": GIGAPUB_PROJECT_ID if gigapub_active() else "",
                "reward": GIGAPUB_REWARD_UNITS if gigapub_active() else 0,
                "daily_limit": GIGAPUB_DAILY_LIMIT,
                "watched_today": gigapub_today_count(u),
            },
            "monetix": {
                "api_key": MONETIX_API_KEY,
                "reward": MONETIX_REWARD_UNITS,
                "daily_limit": MONETIX_DAILY_LIMIT,
                "watched_today": monetix_today_count(u),
            },
            "levels": [
...
@app.route("/api/ads/token", methods=["POST"])
@api
def ads_token(user):
    """Cấp một token dùng một lần, xác nhận người dùng vừa xem xong quảng cáo cho một hành động cụ thể."""
    purpose = (request.get_json(silent=True) or {}).get("purpose")
    if purpose not in ("mine_start", "mine_claim", "monetix_watch"):
        raise ApiError("invalid_purpose", "Không hợp lệ.")
    if purpose == "monetix_watch" and monetix_today_count(user) >= MONETIX_DAILY_LIMIT:
        raise ApiError("daily_limit", "Bạn đã xem đủ %d quảng cáo hôm nay, quay lại vào ngày mai." % MONETIX_DAILY_LIMIT, 409)
    return {"token": issue_ad_token(user["id"], purpose), "expires_in": AD_TOKEN_TTL}


@app.route("/api/ads/postback")
def ads_postback():
    """Adsgram gọi GET vào đây khi người dùng xem xong quảng cáo (cấu hình ở ô 'Reward url').
    Đây chỉ là tín hiệu ghi log để bạn theo dõi (mục Nhật ký trong /admin) — Adsgram không ký
    request này nên không dùng để tự cấp thưởng, tránh bị giả mạo bằng cách gọi thẳng URL."""
    raw = request.args.get("userid", "")
    try:
        uid = int(raw)
    except ValueError:
        return ("bad userid", 400)
    cooldown_key = ("ads_postback", uid)
    now = time.time()
    if now - _last_call.get(cooldown_key, 0) < 1:
        return ("ok", 200)   # bỏ qua bản ghi trùng lặp đến dồn dập
    _last_call[cooldown_key] = now
    with tx():
        admin_log("ad_reward_confirmed", uid, "Adsgram xác nhận đã xem quảng cáo")
    return ("ok", 200)


@app.route("/api/ads/gigapub/postback")
def gigapub_postback():
    """Gigapub Support yêu cầu một link postback riêng (khác cơ chế rewardClaim+hash ở dưới),
    dạng GET tới URL này kèm uid và event, không có chữ ký xác thực nào. CHỈ ghi log để bạn
    theo dõi trong /admin — CHƯA dùng để tự cộng xu, vì không xác minh được request có thật
    sự từ Gigapub hay không, và chưa rõ event 'ad_shown' có nghĩa là đã xem xong hay chỉ mới
    bắt đầu hiện quảng cáo. Đăng ký link:
        https://ten-mien-cua-ban/api/ads/gigapub/postback?uid={user_id}&event={event}
    """
    raw = request.args.get("uid", "")
    event = request.args.get("event", "")[:64]
    try:
        uid = int(raw)
    except ValueError:
        return ("bad uid", 400)
    cooldown_key = ("gigapub_postback", uid, event)
    now = time.time()
    if now - _last_call.get(cooldown_key, 0) < 1:
        return ("ok", 200)
    _last_call[cooldown_key] = now
    with tx():
        admin_log("gigapub_postback", uid, "Gigapub postback: %s" % (event or "(không rõ event)"))
    return ("ok", 200)


@app.route("/api/tasks/gigapub/verify", methods=["POST"])
@api
def gigapub_verify(user):
    """Xác minh một phần thưởng Gigapub theo đúng cơ chế chữ ký của họ:
    client chuyển tiếp nguyên dữ liệu sự kiện 'rewardClaim' (rewardId, userId, projectId,
    amount, hash) sang đây. Server tính lại hash bằng khoá bí mật (GIGAPUB_SECRET_KEY) —
    chỉ server của mình và Gigapub biết khoá này — nếu khớp mới coi là quảng cáo đã xem
    thật. `amount` được giữ nguyên dạng chuỗi client gửi lên (JS tự format số trước khi
    gửi) để chữ ký tính ra đúng khớp với chữ ký gốc, không parse lại thành số ở Python.
    Cùng một rewardId gửi lại nhiều lần (Gigapub có thể phát sự kiện trùng) chỉ được cộng
    xu một lần, các lần sau trả lại đúng confirmation_hash đã lưu để client vẫn xác nhận
    lại được với Gigapub."""
    uid = user["id"]
    if not gigapub_active():
        raise ApiError("not_configured", "Nhiệm vụ này chưa được bật.", 404)
    d = request.get_json(silent=True) or {}
    reward_id = str(d.get("rewardId") or "").strip()
    body_uid = str(d.get("userId") or "").strip()
    project_id = str(d.get("projectId") or "").strip()
    amount = d.get("amount")
    given_hash = str(d.get("hash") or "").strip()
    if not reward_id or not body_uid or not project_id or amount is None or not given_hash:
        raise ApiError("invalid_payload", "Thiếu dữ liệu từ Gigapub.")
    amount = str(amount)
    if body_uid != str(uid):
        raise ApiError("uid_mismatch", "Không khớp người dùng.", 403)
    if project_id != GIGAPUB_PROJECT_ID:
        raise ApiError("project_mismatch", "Không khớp project.", 403)

    expected = hashlib.sha1(f"{body_uid}:{project_id}:{reward_id}:{amount}:{GIGAPUB_SECRET_KEY}".encode()).hexdigest()
    if not hmac.compare_digest(expected, given_hash):
        raise ApiError("invalid_hash", "Chữ ký không hợp lệ.", 403)
    confirmation_hash = hashlib.sha1(f"{body_uid}:{project_id}:{reward_id}:{amount}:confirm:{GIGAPUB_SECRET_KEY}".encode()).hexdigest()

    with tx():
        existing = DB.execute("SELECT confirmation_hash, units FROM gigapub_rewards WHERE reward_id=?", (reward_id,)).fetchone()
        if existing:
            return {"confirmation_hash": existing["confirmation_hash"], "reward": 0, "already_claimed": True, "me": me_payload(uid)}
        u = DB.execute("SELECT gigapub_day, gigapub_count FROM users WHERE id=?", (uid,)).fetchone()
        day = today_str()
        count = gigapub_today_count(u)
        if count >= GIGAPUB_DAILY_LIMIT:
            raise ApiError("daily_limit", "Bạn đã xem đủ %d quảng cáo hôm nay, quay lại vào ngày mai." % GIGAPUB_DAILY_LIMIT, 409)
        DB.execute("UPDATE users SET gigapub_day=?, gigapub_count=? WHERE id=?", (day, count + 1, uid))
        add_balance(uid, GIGAPUB_REWARD_UNITS, "gigapub", reward_id if reward_id.isdigit() else None, earned=True)
        try:
            DB.execute(
                "INSERT INTO gigapub_rewards(reward_id, user_id, amount, units, confirmation_hash, created_at) VALUES(?,?,?,?,?,?)",
                (reward_id, uid, amount, GIGAPUB_REWARD_UNITS, confirmation_hash, time.time()),
            )
        except sqlite3.IntegrityError:
            raise ApiError("race", "Vui lòng thử lại.", 409)
    return {"confirmation_hash": confirmation_hash, "reward": GIGAPUB_REWARD_UNITS, "already_claimed": False, "me": me_payload(uid)}


@app.route("/api/tasks/monetix/claim", methods=["POST"])
@api
def monetix_claim(user):
    """Monetix không ký phần thưởng (khác Gigapub) nên chỉ tin được ở mức "client báo đã xem
    xong" — chặn gian lận bằng ad_token dùng-một-lần, giống hệt cách làm với Adsgram."""
    uid = user["id"]
    cooldown(uid, "monetix", 1)
    if not MONETIX_API_KEY:
        raise ApiError("not_configured", "Nhiệm vụ này chưa được bật.", 404)
    token = (request.get_json(silent=True) or {}).get("ad_token")
    if not consume_ad_token(uid, "monetix_watch", token, enabled=True):
        raise ApiError("ad_required", "Bạn cần xem hết quảng cáo.", 402)
    with tx():
        u = DB.execute("SELECT monetix_day, monetix_count FROM users WHERE id=?", (uid,)).fetchone()
        day = today_str()
        count = monetix_today_count(u)
        if count >= MONETIX_DAILY_LIMIT:
            raise ApiError("daily_limit", "Bạn đã xem đủ %d quảng cáo hôm nay, quay lại vào ngày mai." % MONETIX_DAILY_LIMIT, 409)
        DB.execute("UPDATE users SET monetix_day=?, monetix_count=? WHERE id=?", (day, count + 1, uid))
        add_balance(uid, MONETIX_REWARD_UNITS, "monetix", None, earned=True)
    return {"reward": MONETIX_REWARD_UNITS, "me": me_payload(uid)}
...
def mine_start(user):
    ...
    if not consume_ad_token(uid, "mine_start", (request.get_json(silent=True) or {}).get("ad_token"), enabled=bool(ADSGRAM_BLOCK_ID)):
        raise ApiError("ad_required", "Bạn cần xem hết quảng cáo trước khi bắt đầu đào.", 402)
    with tx():
...
def mine_claim(user):
    uid = user["id"]
    cooldown(uid, "mine", 0.5)
    if not consume_ad_token(uid, "mine_claim", (request.get_json(silent=True) or {}).get("ad_token"), enabled=bool(ADSGRAM_BLOCK_ID)):
        raise ApiError("ad_required", "Bạn cần xem hết quảng cáo trước khi nhận thưởng.", 402)
    with tx():
...
# Trang HTML là vỏ tĩnh (không chứa dữ liệu). Mọi dữ liệu và thao tác đều qua /admin/api/*,
# và các API này chỉ chạy khi initData Telegram hợp lệ VÀ user id == ADMIN_ID.
PAGE_SIZE = 20
WD_STATUSES = ("pending", "paid", "rejected")


def admin_uid():
    if DEV_MODE and request.headers.get("X-Dev-User"):
        raw = request.headers["X-Dev-User"].partition(":")[0]
        try:
            uid = int(raw)
        except ValueError:
            raise ApiError("unauthorized", "Dev user không hợp lệ.", 401)
    else:
        parsed = parse_init_data(request.headers.get("X-Init-Data", ""), max_age=ADMIN_SESSION_SECONDS)
        if not parsed:
            raise ApiError("unauthorized", "Hãy mở trang này từ Telegram (phiên có thể đã hết hạn, mở lại app).", 401)
        uid = int(parsed["user"]["id"])
    if uid != ADMIN_ID:
        raise ApiError("forbidden", "Bạn không có quyền truy cập trang này.", 403)
    return uid
...
def admin_api(fn):
    ...
    def wrapper(*a, **kw):
        admin_uid()
        return jsonify(ok=True, **fn(*a, **kw))

    return wrapper


def admin_log(action, target=None, detail=""):
    """Chỉ gọi trong tx()."""
    DB.execute(
        "INSERT INTO admin_log(action, target, detail, ts) VALUES(?,?,?,?)",
        (action, target, (detail or "")[:300], time.time()),
    )
...
def admin_stats():
    def one(sql, args=()):
        return q1(sql, args)[0] or 0

    total_bal = one("SELECT COALESCE(SUM(balance),0) FROM users")
    pend = q1("SELECT COUNT(*) AS c, COALESCE(SUM(usdt_micro),0) AS m, COALESCE(SUM(units),0) AS u FROM withdrawals WHERE status='pending'")
    paid = q1("SELECT COUNT(*) AS c, COALESCE(SUM(usdt_micro),0) AS m FROM withdrawals WHERE status='paid'")
    return {
        "stats": {
            "admin_id": ADMIN_ID,
            ...
            "balance_usdt": fmt_usdt(usdt_micro(total_bal)),
            "pending_count": pend["c"],
            "pending_units": pend["u"],
            "pending_usdt": fmt_usdt(pend["m"]),
            "paid_count": paid["c"],
            "paid_usdt": fmt_usdt(paid["m"]),
            "fee_total": one("SELECT COALESCE(SUM(delta),0) FROM ledger WHERE user_id=0 AND reason='fee'"),
            "upgrade_total": one("SELECT COALESCE(-SUM(delta),0) FROM ledger WHERE reason='upgrade'"),
            "active_campaigns": one("SELECT COUNT(*) FROM campaigns WHERE status='active'"),
        }
    }
...
@admin_api
def admin_users():
    qs = (request.args.get("q") or "").strip()[:64]
    try:
    ...
    if qs:
        if qs.isdigit():
            where.append("(id=? OR name LIKE ? ESCAPE '\\')")
            args += [int(qs), "%" + like_escape(qs) + "%"]
        else:
            where.append("name LIKE ? ESCAPE '\\'")
            args.append("%" + like_escape(qs) + "%")
    if request.args.get("banned") == "1":
        where.append("banned=1")
    w = ("WHERE " + " AND ".join(where)) if where else ""
    total = q1("SELECT COUNT(*) AS c FROM users " + w, args)["c"]
    rows = q(
        "SELECT id, name, balance, total, mine_level, banned, created_at FROM users " + w
...
def admin_user(uid):
    u = q1("SELECT * FROM users WHERE id=?", (uid,))
    if not u:
        raise ApiError("not_found", "Không tìm thấy người dùng.", 404)
    ledger = [
        dict(r)
        for r in q("SELECT id, delta, reason, ref, ts FROM ledger WHERE user_id=? ORDER BY id DESC LIMIT 15", (uid,))
    ]
    wds = [
        dict(r)
        for r in q(
            "SELECT id, units, usdt_micro, address, status, created_at FROM withdrawals WHERE user_id=? ORDER BY id DESC LIMIT 5",
            (uid,),
        )
    ]
    return {
        "user": {
            "id": u["id"], "name": u["name"], "balance": u["balance"], "total": u["total"],
            "collects": u["collects"], "mine_level": u["mine_level"], "banned": bool(u["banned"]),
            "ban_reason": u["ban_reason"], "referred_by": u["referred_by"], "created_at": u["created_at"],
            "mining": u["mine_start"] is not None,
            "friends": q1("SELECT COUNT(*) AS c FROM users WHERE referred_by=?", (uid,))["c"],
            "tasks_done": q1("SELECT COUNT(*) AS c FROM completions WHERE user_id=? AND status='ok'", (uid,))["c"],
            "campaigns": q1("SELECT COUNT(*) AS c FROM campaigns WHERE owner_id=?", (uid,))["c"],
        },
        "ledger": ledger,
        "withdrawals": wds,
    }
...
@app.route("/admin/api/users/<int:uid>/ban", methods=["POST"])
@admin_api
def admin_ban(uid):
    reason = ((request.get_json(silent=True) or {}).get("reason") or "").strip()[:200]
    if uid == ADMIN_ID:
        raise ApiError("cannot_ban_admin", "Không thể khoá tài khoản admin.", 409)
    with tx():
        if not DB.execute("SELECT 1 FROM users WHERE id=?", (uid,)).fetchone():
            raise ApiError("not_found", "Không tìm thấy người dùng.", 404)
        DB.execute("UPDATE users SET banned=1, ban_reason=? WHERE id=?", (reason or None, uid))
        admin_log("ban", uid, reason)
    return {}


@app.route("/admin/api/users/<int:uid>/unban", methods=["POST"])
@admin_api
def admin_unban(uid):
    with tx():
        if not DB.execute("SELECT 1 FROM users WHERE id=?", (uid,)).fetchone():
...
@app.route("/admin/api/users/<int:uid>/balance", methods=["POST"])
@admin_api
def admin_balance(uid):
    ...
    try:
        ...
        raise ApiError("invalid_amount", "Số xu không hợp lệ.")
    if mode not in ("add", "set"):
        raise ApiError("invalid_mode", "Chế độ không hợp lệ.")
    if abs(amount) > 10_000_000:
        raise ApiError("too_large", "Số quá lớn (tối đa 10.000.000 xu mỗi lần).")
    with tx():
        u = DB.execute("SELECT balance FROM users WHERE id=?", (uid,)).fetchone()
        if not u:
            raise ApiError("not_found", "Không tìm thấy người dùng.", 404)
        new = u["balance"] + amount if mode == "add" else amount
        if new < 0:
            raise ApiError("negative", "Số dư không thể âm.", 409)
        delta = new - u["balance"]
        if delta == 0:
            raise ApiError("no_change", "Số dư không thay đổi.", 409)
        add_balance(uid, delta, "admin_adjust", None)
        admin_log("balance", uid, "%s %d: %d -> %d%s" % (mode, amount, u["balance"], new, (" · " + note) if note else ""))
    return {"balance": new}


def wd_mark_paid(wid, txhash):
    if not re.fullmatch(r"0x[0-9a-fA-F]{64}", txhash or ""):
    ...
    with tx():
        ...
        DB.execute("UPDATE withdrawals SET status='paid', txhash=?, updated_at=? WHERE id=?", (txhash, time.time(), wid))
        admin_log("withdraw_paid", wid, txhash)
    notify(w["user_id"], "Đã chuyển %s USDT (BEP20) tới ví %s.\nTx: %s" % (fmt_usdt(w["usdt_micro"]), w["address"], txhash))


def wd_reject(wid, note):
    with tx():
        w = DB.execute("SELECT * FROM withdrawals WHERE id=?", (wid,)).fetchone()
        if not w or w["status"] != "pending":
            raise ApiError("not_pending", "Yêu cầu không tồn tại hoặc đã xử lý.", 409)
        DB.execute("UPDATE withdrawals SET status='rejected', note=?, updated_at=? WHERE id=?", (note, time.time(), wid))
        add_balance(w["user_id"], w["units"], "withdraw_refund", wid)   # hoàn xu
        admin_log("withdraw_reject", wid, note)
...
def admin_withdrawals():
    ...
    if status not in WD_STATUSES:
        raise ApiError("invalid_status", "Trạng thái không hợp lệ.")
    rows = q(
        """SELECT w.*, u.name AS user_name, u.banned AS user_banned,
           ...
           WHERE w.status = ? ORDER BY w.id """ + ("ASC" if status == "pending" else "DESC") + " LIMIT 100",
        (status,),
    )
    return {
        "withdrawals": [
            {
                "id": r["id"], "user_id": r["user_id"], "user": r["user_name"], "user_banned": bool(r["user_banned"]),
                "units": r["units"], "usdt": fmt_usdt(r["usdt_micro"]), "address": r["address"],
                "status": r["status"], "txhash": r["txhash"], "note": r["note"],
                "shared": r["shared"], "created_at": r["created_at"],
            }
            for r in rows
        ]
    }
...
@app.route("/admin/api/withdrawals/<int:wid>/paid", methods=["POST"])
@admin_api
def admin_wd_paid(wid):
    wd_mark_paid(wid, ((request.get_json(silent=True) or {}).get("txhash") or "").strip())
    return {}


@app.route("/admin/api/withdrawals/<int:wid>/reject", methods=["POST"])
@admin_api
def admin_wd_reject(wid):
    wd_reject(wid, ((request.get_json(silent=True) or {}).get("note") or "").strip()[:200])
    return {}
...
@admin_api
def admin_logs():
    rows = q("SELECT id, action, target, detail, ts FROM admin_log ORDER BY id DESC LIMIT 60")
